...
export function isTrackAiInsightReady(sectionId: ScorecardCategory, answers: DetailAnswers): boolean {
  const section = DETAIL_SECTIONS.find(item => item.id === sectionId);
  if (!section) return false;
  return countSectionAnswers(section, answers) >= MIN_TRACK_FIELDS_FOR_AI_INSIGHT;
}
